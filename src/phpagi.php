...
class AGI
{
    ...
    */
    function fastpass_say_digits(&$buffer, $digits, $escape_digits='')
    {
     ...
     {
         if(!strpos(chr(255) . $escape_digits, substr($buffer, strlen($buffer)-1,1)))
           $proceed = true;
     }
     if($buffer == '' || $proceed)
     {
     ...
     return array('code'=>AGIRES_OK, 'result'=>ord(substr($buffer, strlen($buffer)-1,1)));
    }
    ...
    */
    function fastpass_stream_file(&$buffer, $filename, $escape_digits='', $offset=0)
    {
     ...
     return array('code'=>AGIRES_OK, 'result'=>ord(substr($buffer, strlen($buffer)-1,1)), 'endpos'=>0);
    }
    ...
    */
    function parse_callerid($callerid=NULL)
    {
        ...
        if(substr($callerid, 0, 1) == '"' || substr($callerid, 0, 1) == "'")
        {
    ...
    function say_punctuation($text, $escape_digits='', $frequency=8000)
    {
        ...
        for($i = 0; $i < strlen($text); $i++)
        {
          switch(substr($text,$i,1))
          {
            ...
            default: $ret .= substr($text,$i,1) . ' '; break;
          }
        }
    ...
    */
    ...
    //          $this->conlog("evaluate error on read for $command");
        ...
        if(substr($str, 0, 1) == '-') // we have a multiline response!
        {
          $count = 0;
          $str = substr($str, 1) . "\n";
 ...
 */
  function phpagi_error_handler($level, $message, $file, $line, $context)
  {
    ...
    if(function_exists('mail') && !is_null($phpagi_error_handler_email)) // generate email debugging information
    {
        ...
        {
          $c = ord(substr($message, $i , 1));
          if($c == 10 || $c == 13 || $c == 9)
            $ret .= substr($message, $i , 1);
          elseif($c < 16)
            $ret .= '\x0' . dechex($c);
          elseif($c < 32 || $c > 127)
            $ret .= '\x' . dechex($c);
